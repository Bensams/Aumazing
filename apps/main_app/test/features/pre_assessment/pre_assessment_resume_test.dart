...
/// The pre-assessment entry point must ask about an interrupted run *before*
/// starting a new one — starting one closes the run worth offering (AUM-154).
void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  setUpAll(() {
    // The Start over path writes the sensory consent row for the new run.
    sqfliteFfiInit();
  ...
  /// Waits for the transition screen while real work is in flight: the Start
  /// over path writes the consent row to a real (ffi) database, which only
  /// progresses outside the fake-async zone.
  Future<void> pumpUntilPlayable(WidgetTester tester) async {
    for (var i = 0; i < 40; i++) {
      if (find.text('Play!').evaluate().isNotEmpty) return;
  ...
  testWidgets('Continue adopts the run and skips the games already played', (
    ...
    expect(find.text('Game 3 of 4'), findsOneWidget);
