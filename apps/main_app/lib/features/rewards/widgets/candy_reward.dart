...
/// Sine curve for sway animation
/// Maps 0→0 and 1→1 with sine wave oscillation in between
class SineCurve extends Curve {
  final int cycles;
  final double amplitude;
  
  const SineCurve({this.cycles = 3, this.amplitude = 0.3});

  @override
  double transform(double t) {
    // Linear progression from 0 to 1 with sine wobble added
    // At t=0: returns 0, at t=1: returns 1 (with sine(2π * cycles) ≈ 0 when cycles is integer)
  ...
  void _generateCandies() {
    for (var i = 0; i < widget.candyCount; i++) {
      _candies.add(_CandyConfig(
        initialX: 0.08 + _random.nextDouble() * 0.84, // 8% to 92% of screen width
        size: 50 + _random.nextDouble() * 50, // 50-100 size
