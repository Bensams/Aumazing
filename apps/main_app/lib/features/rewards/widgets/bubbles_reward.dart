...
/// Sine curve for wobble animation
/// Maps 0→0 and 1→1 with sine wave oscillation in between
class SineCurve extends Curve {
  final int wobbles;
  final double amplitude;
  
  const SineCurve({this.wobbles = 4, this.amplitude = 0.1});

  @override
  double transform(double t) {
    // Linear progression from 0 to 1 with sine wobble added
    // At t=0: returns 0, at t=1: returns 1 (with sine(2π * wobbles) ≈ 0)
    return t + sin(t * 2 * pi * wobbles) * amplitude;
  }
}
  ...
  void _generateBubbles() {
    for (var i = 0; i < widget.bubbleCount; i++) {
      _bubbles.add(_BubbleConfig(
        initialX: 0.05 + _random.nextDouble() * 0.9, // 5% to 95% of screen width
        size: 40 + _random.nextDouble() * 60, // 40-100 size
